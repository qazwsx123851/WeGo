...
| 面向 | 分數 (1-10) | 說明 |
|------|:-----------:|------|
| 架構設計 | **6.5** | 分層清晰但 TripController (1664行) 違反 SRP，業務邏輯洩漏到 Controller；已提取 BaseWebController 部分改善重複問題 |
...
| 效能 | **6.5** | 資料庫索引已加上、Cache-Control 已設定、Thymeleaf cache 已啟用；N+1 查詢和快取策略仍需改善 |
| 測試覆蓋 | **7** | ~864 單元測試 + ~118 E2E 測試全數通過，核心 Service 皆有測試，但 Web Controller 零測試、缺整合測試 |
| **整體** | **7.0** | 功能完整的 MVP，核心安全無虞，近期修復顯著改善效能與前端品質，但仍需重構架構才適合生產部署 |
...
| 2 | 🔴 | 效能 | `getUserTrips` N+1 查詢 | `TripService.java:185-195` | 每頁 20 筆行程 = 40+ 額外查詢 |
| 3 | 🔴 | 架構 | TripController 1664 行，God Controller | `TripController.java` | 可維護性極差，違反 SRP |
| 4 | 🔴 | 架構 | 業務邏輯洩漏到 Controller 層 | `TripController.java` 多處 | 無法測試、無法復用 |
| 5 | 🔴 | 架構 | Place find-or-create 邏輯重複 77 行 | `TripController.java:1081-1158, 1304-1380` | DRY 違反 |
| 6 | 🔴 | 前端 | `members.html` 引用 `inviteLink`/`inviteLinkExpiry` 但 Controller 未提供 | `trip/members.html` + `TripController` | 邀請連結功能完全損壞 |
...
| 提取 `BaseWebController` 基礎類別 | 架構 | 🟡 Warning | 部分修復 |
...
| 🟡 Warning | 8 | 5 個 Controller 直接注入 Repository、`getCurrentUser()` 重複（已部分修復）、權限檢查樣板重複 11 處、auth 模式不一致 |
...
|--------|:----:|----------|
| 🔴 Critical | 1 | N+1 查詢 |
| 🟡 Warning | 6 | Settlement N+1、無界列表查詢、Trip 刪除 9 次 DB 操作、`getTrip` 無快取、`PermissionChecker` 無快取、CacheService 無上限 |
...
| # | 任務 | 面向 | 預期效果 | 狀態 |
|---|------|------|----------|------|
| 1 | `application.yml` 設定 `include-message: never`, `include-binding-errors: never` | 安全 | 阻止內部資訊洩漏 | 待處理 |
| 2 | 建立 `application-prod.yml`: `ddl-auto: validate`, SQL 日誌 `WARN`, `thymeleaf.cache: true` | 安全+效能 | 生產環境安全設定 | 部分完成（Thymeleaf cache 已啟用） |
| 3 | 修復 `showMembersPage()` 提供 `inviteLink` / `inviteLinkExpiry` 給模板 | 前端 | 邀請連結功能恢復 | 待處理 |
...
| # | 任務 | 面向 | 預期效果 | 狀態 |
|---|------|------|----------|------|
| 5 | 修復 `getUserTrips` N+1：批次查詢 TripMember + User | 效能 | Dashboard 查詢數從 40+ 降至 3 | 待處理 |
| 6 | 靜態資源加 `Cache-Control` | 效能 | 頁面載入速度顯著提升 | 已完成 |
| 7 | `PermissionChecker` 加 Request-scoped 快取 | 效能 | 每次請求減少 3+ 重複查詢 | 待處理 |
| 8 | 提取 `BaseWebController`：`getCurrentUser()` + `findCurrentMember()` + `canEdit()` | 架構 | 消除 8+ Controller 重複程式碼 | 部分完成 |
...
| # | 任務 | 面向 | 預期效果 | 狀態 |
|---|------|------|----------|------|
| 12 | 拆分 TripController 為 4-5 個 focused Controller | 架構 | SRP 合規、可維護性大幅提升 | 待處理 |
| 13 | 業務邏輯從 Controller 移至 Service 層 | 架構 | 邏輯可測試、可復用 | 待處理 |
| 14 | Place find-or-create 提取至 `PlaceService` | 架構 | 消除 77 行重複 | 待處理 |
| 15 | 統一快取系統至 Spring Cache + Caffeine | 效能 | 消除記憶體洩漏風險、統一管理 | 待處理 |
| 16 | CSP 從 `unsafe-inline` 遷移至 nonce-based | 安全 | XSS 防護等級提升 | 待處理 |
| 17 | 補寫 TodoApiController + ExchangeRateApiController WebMvcTest | 測試 | REST API 100% Controller 測試覆蓋 | 待處理 |
...
| 21 | 補寫 Web Controller 測試 (30+ 端點) | 測試 | Thymeleaf 渲染正確性驗證 | 待處理 |

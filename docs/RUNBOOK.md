...
> 最後更新: 2026-02-20 | 部署平台: Railway
>
> **變更日誌**:
> - 2026-02-20: 前端品質 + 效能改善 — SortableJS 遷移、Settlement 快取、個人記帳功能、測試統計更新 (1152 tests / 12 E2E specs)
> - 2026-02-13: AI 聊天機器人維運 — Gemini API、Circuit Breaker、Rate Limiting
...
| `GEMINI_API_KEY` | Gemini AI API | (Mock) |
| `GEMINI_ENABLED` | 啟用 AI 聊天 | `false` |
...
**檢查**: 執行 `./mvnw jacoco:report` 產生報告，再以瀏覽器開啟 `target/site/jacoco/index.html` 檢視。目前專案有 1152 個單元測試 (86 個測試檔案) 和 12 個 E2E spec。
...
**症狀**: 聊天回覆「AI 助手暫時無法回覆」或無回應

**可能原因**:
- Gemini API Key 未設定或無效
- API 配額用盡
- Circuit Breaker 已開啟（連續 3 次失敗）
- 使用者 Rate Limited（每分鐘 5 次）

**修復**:
1. 確認 `GEMINI_API_KEY` 已設定且有效
2. 確認 `GEMINI_ENABLED=true`
3. 透過 `railway logs` 搜尋 `circuit breaker` 確認斷路器狀態
4. 若斷路器開啟，等待 5 分鐘冷卻後自動恢復
5. 若 API 配額用盡，系統會 fallback 到 MockGeminiClient
...
- [ ] 確認 `GEMINI_ENABLED=true`
- [ ] 檢查 Circuit Breaker 是否開啟 (`railway logs` 搜尋 `circuit breaker`)
- [ ] 檢查使用者 Rate Limit 是否觸發
- [ ] 確認 Gemini API 配額未用盡
...
| 關鍵字 | 監控項目 |
|--------|----------|
| `Gemini` | Gemini API 呼叫狀態 |
| `circuit breaker` | 斷路器開啟/關閉 |
| `RATE_LIMITED` | 聊天限流觸發 |
| `MESSAGE_TOO_LONG` | 訊息過長被拒 |

**配置參數**:
| 參數 | 預設值 | 說明 |
|------|--------|------|
| `chat.rate-limit-per-minute` | 5 | 每使用者每分鐘聊天次數 |
| `gemini.circuit-breaker-failure-threshold` | 3 | 觸發斷路器的連續失敗次數 |
| `gemini.circuit-breaker-cooldown-minutes` | 5 | 斷路器冷卻時間 |
| `gemini.max-output-tokens` | 1500 | AI 回覆最大 token 數 |
...
| `RATE_LIMITED` | 400 | 聊天請求過於頻繁 |
| `MESSAGE_TOO_LONG` | 400 | 訊息超過 byte 限制 |
| `CIRCUIT_BREAKER_OPEN` | 500 | AI 服務暫時不可用 |
...
**測試統計**: 1152 個單元測試 (86 個測試檔案)，12 個 E2E spec
...
| `member.spec.ts` | 成員管理 | ~20s |
| `settlement.spec.ts` | 結算流程 | ~25s |
| `profile.spec.ts` | 個人檔案 | ~15s |
| `statistics.spec.ts` | 支出統計 | ~20s |
| `personal-expense.spec.ts` | 個人記帳 | ~30s |
...
| 快取名稱 | TTL | 最大條目 | 說明 |
|----------|-----|---------|------|
| `exchangeRates` | 1 小時 | 100 | 匯率快取 |
| `weather` | 30 分鐘 | 500 | 天氣預報快取 |
| `settlement` | 1 分鐘 | 200 | 結算快取 (支出變動時 eviction) |
| `rateLimitBuckets` | 5 分鐘 | 100,000 | IP Rate Limiting |
| `chatRateLimit` | 2 分鐘 | 10,000 | 聊天限流 |
| `permissionCheck` | 5 秒 | - | 權限檢查 (請求級快取) |

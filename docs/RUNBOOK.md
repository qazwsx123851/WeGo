...
**檢查**: 執行 `./mvnw jacoco:report` 產生報告，再以瀏覽器開啟 `target/site/jacoco/index.html` 檢視。目前專案有 ~1060 個單元測試和 11 個 E2E spec。
